...
# 修正:
# - データ構造に "title" フィールドを追加
# - add: titleを空文字列で初期化
# - load: titleフィールドを読み込む（存在しない場合は空文字列）
# - touch_last_check: series_titleをオプションで受け取り更新
...
class FavoritesStore:
                ...
                if isinstance(item, dict):
                    ...
                    if isinstance(url, str):
                        out[url] = {
                            ...
                            "title": item.get("title", "") # ✅ title 読み込み
                        }
        self._data = out
    ...
    def touch_last_check(self, series_url: str, series_title: Optional[str] = None) -> None:
        ...
        if u not in self._data:
            ...
            # ✅ 受け取ったシリーズタイトルがあり、既存タイトルと異なる場合のみ更新
